...
export function renderTeamGrid() {
  ...
  grid.innerHTML = members.map(m => {
    ...
    const avatar = m.photoURL
      ? `<img src="${escapeHtml(m.photoURL)}" alt="" class="w-10 h-10 rounded-full object-cover border border-white/10" onerror="this.outerHTML = '<div class=\'w-10 h-10 rounded-full bg-gradient-to-br from-purple to-cyan flex items-center justify-center text-[13px] font-bold text-white shadow-lg shadow-black/20\'>${escapeHtml(initials)}</div>';" />`
      : `<div class="w-10 h-10 rounded-full bg-gradient-to-br from-purple to-cyan flex items-center justify-center text-[13px] font-bold text-white shadow-lg shadow-black/20">${escapeHtml(initials)}</div>`;

    return `
      <div class="team-card bg-elevated border border-white/[0.04] rounded-xl p-4 transition-all hover:bg-hover hover:translate-y-[-2px] hover:shadow-xl hover:shadow-black/40">
        <div class="flex items-start gap-3 mb-3">
          <div class="relative flex-shrink-0">
            ${avatar}
            <div class="absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-elevated ${statusColors[status] || 'bg-gray-500'}"></div>
          </div>
          <div class="min-w-0 flex-1">
            <div class="flex items-center justify-between">
              <h3 class="text-[14px] font-bold text-gray-100 truncate">${escapeHtml(name)}</h3>
            ...
            <p class="text-[12px] text-gray-500 truncate">${escapeHtml(m.email || "")}</p>
          </div>
        </div>
        <div class="flex items-center justify-between mb-4">
          <span class="px-2 py-0.5 rounded-md text-[10.5px] font-bold ${getRoleClass(m.role || "Member")}">${escapeHtml(m.role || "Member")}</span>
          <span class="text-[11px] text-gray-600 font-medium">Joined ${formatDate(joinedAt)}</span>
        </div>
        <div class="mb-4">
          <p class="text-[9.5px] font-bold uppercase tracking-wider text-gray-600 mb-1.5">Projects</p>
          <div class="flex flex-wrap gap-1.5">
            ${memberProjects.length ? memberProjects.map(p => `<span class="px-2 py-0.5 bg-white/5 border border-white/5 rounded text-[10px] text-gray-400 font-medium">${escapeHtml(p.name)}</span>`).join('') : '<span class="text-[10px] text-gray-600 italic">No projects</span>'}
          </div>
        </div>
        <div class="flex items-center justify-between pt-3 border-t border-white/[0.03]">
          <div class="flex items-center gap-1.5 text-gray-500">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="text-cyan"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
            <span class="text-[11px] font-medium font-mono">${taskCount} project tasks</span>
          </div>
          <div class="flex items-center gap-1.5">
            <div class="w-1.5 h-1.5 rounded-full ${statusColors[status] || 'bg-gray-500'}"></div>
            <span class="text-[11px] font-bold capitalize text-gray-400">${status}</span>
          </div>
        </div>
      </div>`;
  }).join("");
